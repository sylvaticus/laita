...
- **`browser/dist-chrome/` and `vscode/core/{anchor,ollama}.js` are no longer committed**
  (5,480 lines removed). They are generated by `build-chrome.sh` and `sync-core.sh`, which
  the tests that check them now run themselves. Chrome users take the zip from a GitHub
  release; pushing a `v*` tag builds and attaches all three packages.
